...
public class RAMStorage extends Storage {
	private Logger logger = LoggerFactory.getLogger(RAMStorage.class);
	...
	public boolean push(Line[] lines, int size) {
		...
		try {
			while (!mars.offer(lines, size, waitTime, TimeUnit.MILLISECONDS)) {
                ...
                if (getDestructLimit() > 0 && getStat().getLineRRefused() >= getDestructLimit()){
                	if (!isPushClosed()){
                		logger.warn("Close RAMStorage for " + getStat().getId() + ". Queue:" + info() + " Timeout times:" + getStat().getLineRRefused());
                        setPushClosed(true);
                	}
                    return false;
                }
