...
/**
 * <code>{@link JobScheduler4OnceImpl}</code>
 *
 * TODO : document me
 *
 * @author kewn
 */
public class JobScheduler4OnceImpl implements JobScheduler {
	
	private static ExecutorService executorService = 
			Executors.newFixedThreadPool(JobSchedulerConf.RUNONCE_JOB_COUNT);
	
	private static ConcurrentMap<String, JobWrapper> jobWrapperMap = new ConcurrentHashMap<String, JobWrapper>();

	
	public JobScheduler4OnceImpl() {
	}
		...
		executorService.execute(new Runnable() {
			...
			public void run() {
				...
				if (jobWrapperMap.containsKey(jobId)) {
					jobWrapper = jobWrapperMap.get(jobId);
				}
				else {
					JobInfo jobInfo = JobStoreFactory.get().selectJob(jobId);
					jobWrapper = JobWrapperBuilder.newBuilder(jobInfo.getExecName(), 
									jobId, jobInfo.getExecParam()).build();
					jobWrapperMap.putIfAbsent(jobId, jobWrapper);
					jobWrapper.init();
				}
